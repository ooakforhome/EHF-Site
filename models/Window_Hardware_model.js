...
const windowHardwareSchema = new Schema({
  Product_Name: {type: String},
  category_type: { type: String, default: "Window Hardware" },
  SKU: {type: String},
  images: {type: String},
...
const WindowHardware = module.exports = mongoose.model('WindowHardware', windowHardwareSchema);

module.exports= {
    getWindowHardwares: function(req, res) {
      WindowHardware
        .find(req.query)
    ...
    // add
    addWindowHardware: function(req, res) {
      WindowHardware
        .create(req.body)
        .then(dbModel => res.json(dbModel))
        .catch(err => res.status(422).json(err));
      },
    getWindowHardwareById: function(req, res) {
      WindowHardware
        .findById(req.params._id)
        .then(dbModel => res.json(dbModel))
        .catch(err => res.status(422).json(err));
      },
    updateWindowHardware: function(req, res) {
      WindowHardware
        .findOneAndUpdate({ _id: req.params._id }, req.body)
        .then(dbModel => res.json(dbModel))
        .catch(err => res.status(422).json(err));
      },
    deleteWindowHardware: function(req, res) {
      WindowHardware
